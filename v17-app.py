...
def show_custom_message(texto, tipo='info'):
    cor_fundo = "#FFC107" if tipo == 'warning' else "#4CAF50" # Amarelo ou Verde
    cor_texto = "#000000" if tipo == 'warning' else "#FFFFFF"
...
with col_form:
    ...
    with st.form("painel_controle", border=False):
        ...
        # CAMPOS (Agora vinculados corretamente às colunas)
...
for campo in campos_data:
    valor = st.session_state.get(campo)
    if valor and valor != DATA_PADRAO:
        filtro_ativo = True
