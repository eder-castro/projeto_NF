...
<style>
    ...
    /* UPLOAD COMPACTO E CENTRALIZADO */
    ...
    /* Ajuste fino para os botões do formulário não estourarem */
...
# --- TOPO (ALINHAMENTO CENTRAL) ---
# vertical_alignment="center" garante que o texto fique na mesma linha do botão de upload
...
if uploaded_file and uploaded_file.name != st.session_state['ultimo_arquivo']:
    ...
    # Atualiza para data de hoje ao extrair (ou mantem 1900 se preferir conferência total)
...
with col_form:
    ...
    with st.form("painel_controle", border=False):
        ...
        c_head_title, c_head_btns = st.columns([0.35, 0.65], vertical_alignment="center")
        
        with c_head_title:
             st.markdown("##### :material/edit_document: Dados")
        
        with c_head_btns:
            # Botões ajustados para não estourarem
            # use_container_width=True faz eles preencherem a coluna, 
            # mas como dividi em 3 colunas pequenas, eles ficam menores.
        ...
        # FORMATO BRASILEIRO: format="DD/MM/YYYY"
        ...
        # Campo Envio Pagamento agora é Data
        ...
        if save_clk:
            if not dados['num_nota']:
                ...
                # Conversão segura de datas para string (YYYY-MM-DD para o banco)
...
column_cfg = {
    "envio_pgto": st.column_config.DateColumn("Envio Pgto.", format="DD/MM/YYYY"), # Agora é data
    "emp_tomador": "Emp",
    "cnpj_tomador": "ERP2",
    "num_requisicao": "RC",
...
# ORDEM EXATA DAS COLUNAS (conforme sua imagem + campos sistema ocultos)
ordem_colunas = [
    "envio_pgto", 
    "emp_tomador", 
    "cnpj_tomador", 
...
df_editado = st.data_editor(
    ...
    column_order=ordem_colunas, # Força a ordem visual
    hide_index=True,
    height=300
)
...
if not df.equals(df_editado):
    if st.button("Gravar Alterações da Tabela", icon=":material/save_as:"):
        ...
        for i, reg in enumerate(registros):
            ...
            # Converte as 3 datas para String ISO antes de salvar
            for campo_data in ['data_emissao', 'data_vencimento', 'envio_pgto']:
