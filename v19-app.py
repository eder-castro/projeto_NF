...
# Estas funções rodam ANTES da tela ser redesenhada, evitando o erro de widget

def reset_form_callback():
    """Limpa todos os campos do estado"""
...
def salvar_callback():
    ...
    # Chama o reset aqui dentro, pois estamos num callback (seguro!)
    for c in campos_texto: st.session_state[c] = ""
    for c in campos_valor: st.session_state[c] = 0.0
    for c in campos_data: st.session_state[c] = DATA_PADRAO

def buscar_callback():
    """Apenas define a mensagem, o filtro acontece no reload"""
...
with col_form:
    ...
    with st.form("painel_controle", border=False):
        ...
        with c_head_btns:
            ...
            # Usamos on_click=NOME_DA_FUNCAO (sem parênteses)
            b_clear.form_submit_button(
                "Limpar", 
                type="secondary", 
                use_container_width=True, 
                icon=":material/delete_sweep:",
                on_click=reset_form_callback
            )
            
            b_search.form_submit_button(
                "Buscar", 
                type="secondary", 
                use_container_width=True, 
                icon=":material/search:",
                on_click=buscar_callback
            )
            
            b_save.form_submit_button(
                "Salvar", 
                type="primary", 
                use_container_width=True, 
                icon=":material/save:",
                on_click=salvar_callback
            )
